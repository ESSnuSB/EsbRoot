...
G4VPhysicalVolume* FgdDetectorConstruction::Construct()
{
  ...
  Double_t cube_Y = fdetector.ParamAsDouble(data::superfgd::DP::length_Y) * lunit;
  Double_t cube_Y_N = fdetector.ParamAsDouble(data::superfgd::DP::number_cubes_Y);
  G4double lengthY =  cube_Y * cube_Y_N;

  Double_t cube_Z = fdetector.ParamAsDouble(data::superfgd::DP::length_Z) * lunit;
  ...
  // Fiber             ------------------------------------------------------
  G4Material* fiberMaterial = nist->FindOrBuildMaterial(fdetector.ParamAsString(data::superfgd::DP::fiber_g4material));
  Double_t fiber_radius = fdetector.ParamAsDouble(data::superfgd::DP::fiber_radius) * lunit;
  ...
  G4Material* scintillator = nist->FindOrBuildMaterial(fdetector.ParamAsString(data::superfgd::DP::scintillator));
  if(fdetector.ParamAsBool(data::superfgd::DP::optPhotonOn))
  {
  ...
  // TODO2 - Check how to add the sensitive detector
  /*
  ...
  CubeSD *cubeSD = new CubeSD(cubeSDname , data::superfgd::DP::NF_DETECTOR_CUBE_SD);
  ...
  */
  ...
  // Visualization - Start
  //-------------------------------------------------------------------------
  bool isDetailed = (fdetector.ParamAsInt(data::superfgd::DP::visdetail) > 0);
