void run_sim(Int_t nEvents = 100, TString mcEngine = "TGeant4")
{
  ...
  FairModule* magnet = new EsbMagnet("Magnet");
  run->AddModule(magnet);

  FairModule* pipe = new EsbPipe("Pipe");
  run->AddModule(pipe);
    
  FairDetector* NewDet = new EsbEsb("TestDetector", kTRUE);
 ...
 // ------------------------------------------------------------------------
    ...
    // -----   Magnetic field   -------------------------------------------
    // Constant Field
    EsbConstField  *fMagField = new EsbConstField();
