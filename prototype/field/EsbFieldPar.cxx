...
// ------   Constructor   --------------------------------------------------
EsbFieldPar::EsbFieldPar(const char* name, const char* title,
			 const char* context) 
...
// ------   Put parameters   -----------------------------------------------
void EsbFieldPar::putParams(FairParamList* list) {
...
// --------   Get parameters   ---------------------------------------------
Bool_t EsbFieldPar::getParams(FairParamList* list) {
...
void EsbFieldPar::SetParameters(FairField* field) {

  if ( ! field ) {
    cerr << "-W- EsbFieldPar::SetParameters: Empty field pointer!" << endl;
    return;
  }
  ...
  if ( fType == 0 ) {                                 // constant field
    EsbConstField* fieldConst = (EsbConstField*) field;
    ...
    cerr << "-W- EsbFieldPar::SetParameters: Unknown field type "
	 << fType << "!" << endl;
    fBx = fBy = fBz = 0.;
