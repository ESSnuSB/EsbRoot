...
EsbConstField::EsbConstField(const char* name, Double_t xMin, 
...
EsbConstField::EsbConstField(EsbFieldPar* fieldPar)
  : FairField(),
    fXmin(0.),   
    fXmax(0.),
    fYmin(0.),
...
{
  if ( ! fieldPar ) {
    cerr << "-W- EsbConstField::EsbConstField: empty parameter container!"
	 << endl;
    fType=0;
  }
  else {
...
// -----   Set field region   ----------------------------------------------
void EsbConstField::SetFieldRegion(Double_t xMin, Double_t xMax, 
...
// -----   Set field values   ----------------------------------------------
void EsbConstField::SetField(Double_t bX, Double_t bY, Double_t bZ) {
  fBx   = bX;
  fBy   = bY;
  fBz   = bZ;
}
...
Double_t EsbConstField::GetBx(Double_t x, Double_t y, Double_t z) {
  if ( x < fXmin  ||  x > fXmax  ||
       y < fYmin  ||  y > fYmax  ||
...
Double_t EsbConstField::GetBy(Double_t x, Double_t y, Double_t z) {
  if ( x < fXmin  ||  x > fXmax  ||
       y < fYmin  ||  y > fYmax  ||
       z < fZmin  ||  z > fZmax ) return 0.;
  return fBy;
...
Double_t EsbConstField::GetBz(Double_t x, Double_t y, Double_t z) {
  if ( x < fXmin  ||  x > fXmax  ||
       y < fYmin  ||  y > fYmax  ||
       z < fZmin  ||  z > fZmax ) return 0.;
  return fBz;
